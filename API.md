# Fitflare Personal API

> 此文件由 common/api_docs.py 生成。请修改接口目录后运行 python3 scripts/docs.py。

基础路径：`/api/public/v1`。全部 24 个接口均为 GET。

## 快速开始

通过 /me 直接读取自己的账户，无需档案 ID。私有模式先在仪表盘登录；YOUR_HOST 替换为部署域名。命令行示例未携带登录会话。

```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me'
```
...
`GET /api/public/v1/me/dashboard`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/dashboard'
```

### 数据目录

`GET /api/public/v1/me/catalog`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/catalog'
```

### 概览

`GET /api/public/v1/me/overview`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/overview'
```

### 覆盖范围

`GET /api/public/v1/me/coverage`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/coverage'
```

### 指标列表

`GET /api/public/v1/me/metrics`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/metrics'
```

### 单项指标

`GET /api/public/v1/me/metrics/{metric_key}`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/metrics/steps'
```

### 相关性

`GET /api/public/v1/me/correlations`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/correlations'
```
...
`GET /api/public/v1/me/series/{granularity}`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/series/daily?metrics=sleep_hours,steps,hrv&limit=30'
```

### SVG 趋势图

`GET /api/public/v1/me/charts/{chart_key}.svg`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/charts/series.svg?metrics=sleep_hours,steps,hrv&granularity=daily&limit=30&width=960&height=320&theme=light'
```
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/datasets'
```

### 读取数据集

`GET /api/public/v1/me/datasets/{dataset}`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/datasets/activity?offset=0&limit=20'
```

### 摘要分区列表

`GET /api/public/v1/me/sections`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/sections'
```

### 单个摘要分区

`GET /api/public/v1/me/sections/{section_key}`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/sections/activity'
```

### 记录表目录

`GET /api/public/v1/me/tables`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/tables'
```

### 读取记录表

`GET /api/public/v1/me/tables/{table_key}`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/tables/sleep?offset=0&limit=20'
```
...
`GET /api/public/v1/me/snapshot-status`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/snapshot-status'
```

### 完整快照

`GET /api/public/v1/me/snapshot`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/snapshot'
```

### 快照端点目录

`GET /api/public/v1/me/snapshot/endpoints`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/snapshot/endpoints'
```

### 单个快照端点

`GET /api/public/v1/me/snapshot/endpoints/{endpoint_key}`
...
```sh
curl -fsS 'https://YOUR_HOST/api/public/v1/me/snapshot/endpoints/profile'
```
